...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        # Unpack and move data for the first position in the pair to the device
        piece1, side1, ep1, cas1, fif1, tgt1, d_idx1 = (d.to(self.device) for d in batch1)
        # Unpack and move data for the second position in the pair to the device
        piece2, side2, ep2, cas2, fif2, tgt2, d_idx2 = (d.to(self.device) for d in batch2)

        # Two forward passes, one for each position in the pair
        _, p_win1 = self.model(piece1, side1, ep1, cas1, fif1, d_idx1)
        _, p_win2 = self.model(piece2, side2, ep2, cas2, fif2, d_idx2)

        # --- Custom Loss Calculation ---
        
        # 1. Score Loss: MSE for correctly predicting the score of each position individually.
        loss_score1 = F.mse_loss(p_win1, tgt1)
        loss_score2 = F.mse_loss(p_win2, tgt2)
        total_score_loss = loss_score1 + loss_score2

        # 2. Difference Loss: Weighted MSE for correctly predicting the *difference* in scores.
        pred_diff = p_win1 - p_win2
        target_diff = tgt1 - tgt2
        
        # Weight is inversely proportional to the target difference, capped for stability.
        weight = 1.0 / torch.clamp(torch.abs(target_diff), min=0.0025)
        
        # We apply the weight to the squared error: weight * (pred_diff - target_diff)^2
        diff_mse_unweighted = F.mse_loss(pred_diff, target_diff, reduction='none')
        weighted_diff_loss = (weight * diff_mse_unweighted).mean()

        # 3. Final Combined Loss
        total_loss = (self.config['score_loss_weight'] * total_score_loss +
                      self.config['diff_loss_weight'] * weighted_diff_loss)
        ...
        mae1 = F.l1_loss(p_win1, tgt1)
        mae2 = F.l1_loss(p_win2, tgt2)
        avg_mae = (mae1 + mae2) / 2.0
        
        self.loss_hist.append(total_loss.item())
        self.mae_hist.append(avg_mae.item())
        self.step_count += 1

        return total_loss.item(), avg_mae.item()
