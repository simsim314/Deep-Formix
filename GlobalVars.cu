#include "chess.h"
#include "movegen_device.cuh" // Includes the 'extern' declarations for device variables

// ============================================================================
// HOST-SIDE DEFINITIONS for lookup tables.
// These are standard C++ arrays that exist on the host (CPU). They are
// initialized and used by the MoveGeneratorBitboard::init() function to
// pre-calculate all necessary data, which is then copied to the GPU.
// ============================================================================
...
//FancyMagicEntry bishop_magics_fancy[64] = { /* ... keep the long initializer list ... */ };
//FancyMagicEntry rook_magics_fancy[64] = { /* ... keep the long initializer list ... */ };
uint8  fancy_byte_magic_lookup_table[97264];
uint64 fancy_byte_RookLookup[4900];
uint64 fancy_byte_BishopLookup[1428];


// ============================================================================
// DEFINITIONS FOR GLOBAL GPU DEVICE-SIDE LOOKUP TABLES
// This is the single source file where the memory for the global device-side
// variables is actually allocated. Every other file that needs to access
// these variables should include "movegen_device.cuh" to get the 'extern'
// declarations.
// ============================================================================

__device__ ZobristRandoms   gZob;
__device__ ZobristRandoms   gZob2;
__device__ uint64 gBetween[64][64];
__device__ uint64 gLine[64][64];
__device__ uint64 gRookAttacks[64];
__device__ uint64 gBishopAttacks[64];
__device__ uint64 gQueenAttacks[64];
__device__ uint64 gKingAttacks[64];
__device__ uint64 gKnightAttacks[64];
__device__ uint64 gpawnAttacks[2][64];
__device__ uint64 gRookAttacksMasked[64];
__device__ uint64 gBishopAttacksMasked[64];
__device__ uint64 gRookMagics[64];
__device__ uint64 gBishopMagics[64];
__device__ uint64 gRookMagicAttackTables[64][1 << ROOK_MAGIC_BITS];
__device__ uint64 gBishopMagicAttackTables[64][1 << BISHOP_MAGIC_BITS];
__device__ uint64 g_fancy_magic_lookup_table[97264];
__device__ FancyMagicEntry g_bishop_magics_fancy[64];
__device__ FancyMagicEntry g_rook_magics_fancy[64];
__device__ uint8  g_fancy_byte_magic_lookup_table[97264];
__device__ uint64 g_fancy_byte_BishopLookup[1428];
__device__ uint64 g_fancy_byte_RookLookup[4900];
